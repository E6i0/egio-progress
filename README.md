...
<body class="max-w-md mx-auto p-5 pb-24">
    ...
    <section class="card p-4 mb-4">
        ...
        <div class="flex justify-between px-2 mb-4" id="week-dots">
        ...
        <div class="flex justify-between items-center gap-3">
            <div class="w-full bg-zinc-800 h-1 rounded-full overflow-hidden">
                <div id="year-progress-bar" class="bg-zinc-600 h-full w-0"></div>
            </div>
            <p id="year-percent" class="text-[8px] text-muted">0%</p>
        </div>
    </section>
    ...
    <section class="card p-6 mb-6 border-l-4 border-l-[#d2a160] shadow-2xl">
        <div class="flex justify-between items-end mb-4">
            <div>
                <p class="text-[10px] text-muted uppercase tracking-widest">Puntos Hoy</p>
                <div class="flex items-end gap-2">
                    <h2 id="main-score" class="text-6xl font-extrabold text-white leading-none">0</h2>
                    <span class="text-xs font-bold text-muted pb-1">/ 220 pts</span>
                </div>
            </div>
        ...
        <div class="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden">
            <div id="daily-progress-bar" class="progress-fill h-full w-0"></div>
        </div>
    </section>

    <section class="flex gap-2 mb-8">
        <button onclick="toggleEditMode()" id="btn-edit" class="btn w-1/2 p-3 rounded-lg uppercase tracking-widest">✏️ Editar Misiones</button>
        <button onclick="saveBackup()" class="btn w-1/2 p-3 rounded-lg uppercase tracking-widest">💾 Respaldar Datos</button>
    </section>

    <main id="tasks-root" class="space-y-8"></main>

    <button onclick="addCoin()" class="fixed bottom-6 right-6 btn-gold btn w-14 h-14 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition shadow-accent/20">
    ...
    <script>
        ...
        let gioData = JSON.parse(localStorage.getItem('gioData_v4.1')) || {
            ...
            tasks: {
                morning: [
                    ...
                    { id: "m2", name: "Rutina Higiene Care", pts: 10 },
                    { id: "m3", name: "BIBLIO-STRATEGY (Guía del día)", pts: 15 },
                    { id: "m-extra", name: "Extra Mañana...", pts: 15, isExtra: true }
                ],
                focusBlock: [
                    { id: "f1", name: "07:30AM: Salida Z150 (Manejo Táctico)", pts: 20 },
                    { id: "f2", name: "BAMX: Deep Work en Procuración", pts: 30 }
                ],
                afternoon: [
        ...
        function setupCalendarAndYear() {
            ...
            const remaining = Math.floor((end - DATE) / (1000*60*60*24));
            const percent = Math.floor(((DATE - start) / (end - start)) * 100);
            document.getElementById('days-left-counter').innerText = remaining + ` DÍAS RESTANTES DE ${year}`;
        ...
        function renderTasks() {
            ...
            root.innerHTML = "";
            const sections = [
                { title: "🌞 Mañana de Poder", data: gioData.tasks.morning },
                { title: "🎯 Enfoque & Conducción", data: gioData.tasks.focusBlock, accent: "text-accent" }, 
                { title: "🌤 Tarde de Impacto", data: gioData.tasks.afternoon },
                { title: "🌙 Cierre & Orden", data: gioData.tasks.evening }
            ];
            sections.forEach(s => {
                const group = document.createElement('div');
                group.innerHTML = `<h3 class="text-[9px] ${s.accent || 'text-muted'} mb-3 uppercase tracking-widest font-bold text-white opacity-50">${s.title}</h3>`;
        ...
        function createTaskCard(t) {
            ...
            if (isEditMode || (t.isExtra && !done)) {
                card.innerHTML = `<div class="w-full"><input type="text" value="${t.name}" class="task-input text-sm ${t.isExtra ? 'extra-input text-accent' : ''}" onchange="updateTaskText('${t.id}', this.value)" onclick="event.stopPropagation()"></div><span class="text-[10px] font-bold text-muted ml-2">+${t.pts}</span>`;
            } else {
                card.innerHTML = `<span class="text-sm font-medium">${t.name}</span><span class="text-[10px] font-bold text-muted">+${t.pts}</span>`;
            }
            card.onclick = () => { if(!gioData.completed.includes(t.id)) { gioData.completed.push(t.id); gioData.score += t.pts; updateUI(); saveData(); renderTasks(); } };
            return card;
        }
